VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} FrmPHOPA 
   ...
   ClientWidth     =   11985
   ControlBox      =   0   'False
   Icon            =   "FrmPHOPA.dsx":0000
   MaxButton       =   0   'False
   MinButton       =   0   'False
   OleObjectBlob   =   "FrmPHOPA.dsx":1084A
   StartUpPosition =   2  'CenterScreen
End
Attribute VB_Name = "FrmPHOPA"
...
Exit Sub
End If
If OptionButton9.Value = False And OptionButton10.Value = False And OptionButton43.Value = False Then
...
Exit Sub
End If
If OptionButton39.Value = False And OptionButton40.Value = False And OptionButton44.Value = False Then
...
SQLTambah = "insert into tb1 (TANGGAL,NOMESIN,NIK,Shift,TekananUdaraUtama,TekananUdaraFeedBelt,TekananUdaraClampA,TekananUdaraClampB,TekananUdaraRubberSealUnit,KebersihanMesindanAplicator,PotOilPA5,PakuPengaman,PenggulungKertasTerminal,LampuAndonMesin,HasilCuttingStripping,KondisiRollet,ChipInsulationBoxTerminalBox,MarkershunkApplikator,KondisiStraightmeter,SafetyCover,UkuranCircuitYangTerproses,TabungRubberSeal,TidakAdaEquitmentYangMenghalangiOperator,PanjangWireDatum,Keterangan) values ('" _
& TextBox3 & "','" & ComboBox1 & "','" & TextBox4 & "','" & ComboBox3 & "','" & TekananUtama & "','" & TekananFB & "','" & TekananCA & "','" & TekananCB & "','" & TekananRS & "','" & KebersihanM & "','" & PA5 & "','" & Paku & "','" & Penggulung & "','" & Lampu & "','" & Hasil & "','" & KonRollet & "','" & Chiping & "','" & Marker & "','" & KonStraight & "','" & Safety & "','" & Ukuran & "','" & TabungRS & "','" & Equitment & "','" & Datum & "','" & TextBox2 & "')"

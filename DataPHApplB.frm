...
Begin VB.Form DataPHApplB 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Record Patroli Harian Aplikator"
   ClientHeight    =   6870
   ClientLeft      =   210
   ClientTop       =   555
   ClientWidth     =   9870
   ControlBox      =   0   'False
   Icon            =   "DataPHApplB.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   693.94
...
rs1.Open "select * from Appl_B where Tanggal = '" & Text1 & "'", kon
If Not rs1.EOF Then
    Text2 = rs1!NoMesin
         With DataGrid1
   Set .DataSource = rs1
...
Dim hps As String
hps = "delete from Appl_B where Tanggal = '" & Text1 & "' and Jam = '" & Text3 & "'"
kon.Execute hps
...
rs1.Open "select*from Appl_B", kon, adOpenDynamic, adLockOptimistic
If Text1 = DataGrid1.Columns(0) Then
DataGrid1.Columns(21) = Combo1.Text
...
kon.Open App.Path & "\DbPHAppl.mdb"
Call Histry

DataGrid1.Refresh
DoEvents

rs1.Sort = DataGrid1.Columns(32).DataField & " DESC"

'Combo1.Text = DataGrid1.Columns(21)
